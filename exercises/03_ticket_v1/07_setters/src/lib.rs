...
impl Ticket {
    pub fn new(title: String, description: String, status: String) -> Ticket {
        Self::validate_title_by_zero_length(&title);
        Self::validate_title_by_length(&title);
        Self::validate_description_by_zero_length(&description);
        Self::validate_description_by_length(&description);
    ...
    fn validate_title_by_zero_length(title: &str) {
        if title.is_empty() {
            panic!("Title cannot be empty");
        }
    }

    fn validate_title_by_length(title: &str) {
        if title.len() > 50 {
            panic!("Title cannot be longer than 50 bytes");
        }
    }

    fn validate_description_by_zero_length(description: &str) {
        if description.is_empty() {
            panic!("Description cannot be empty");
        }
    }

    fn validate_description_by_length(description: &str) {
